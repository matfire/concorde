...
>(
  ...
  channel: Ch,
  event: Ev,
  handler: (data: OutType<Registry, Ch, Ev>) => void
) {
  return handler
}
...
const handler = createEventHandler(
  ...
  'user-joined',
  (data) => {
    // data is typed as the output type of the user-joined event
    console.log(data.username) // TypeScript knows this exists
  }
)
```

### Generic Channel Service

```typescript
class ChannelService<Registry> {
  constructor(
    private client: TypedPusherClient<Registry>,
    private server: TypedPusherServer<Registry>
  ) {}

  async triggerAndBind<
    Ch extends ChannelSpecifier<Registry>,
    Ev extends EventNames<Registry, Ch>
  >(
    channelSpec: Ch,
    ...
    // Trigger on server
    await this.server.trigger(channelSpec, event, data)
    
    // Bind on client
    const channel = this.client.subscribe(channelSpec)
...
```typescript
function isStaticChannel<Registry>(
  channelSpec: ChannelSpecifier<Registry>
): channelSpec is StaticChannelNames<Registry> {
  return typeof channelSpec === 'string'
}

function isDynamicChannel<Registry>(
  channelSpec: ChannelSpecifier<Registry>
): channelSpec is DynamicChannelSpec<Registry> {
  return typeof channelSpec === 'object' && 
         channelSpec !== null &&
         'template' in channelSpec &&
         'params' in channelSpec
}
```
...
```typescript
// Use these patterns to test your registry types at compile time

type TestRegistry = typeof registry

// Test channel names
type _StaticChannels = StaticChannelNames<TestRegistry>
type _DynamicChannels = DynamicChannelNames<TestRegistry>
type _AllChannels = ChannelNames<TestRegistry>

// Test event names
type _UserEvents = EventNames<TestRegistry, 'user-events'>
type _DynamicUserEvents = EventNames<TestRegistry, {
  template: 'user-{userId}',
  params: { userId: string }
}>

// Test data types
type _MessageInput = InType<TestRegistry, 'chat-room', 'message'>
type _MessageOutput = OutType<TestRegistry, 'chat-room', 'message'>

// Ensure types match expectations
const _testInput: _MessageInput = {
  content: 'hello',
  userId: 'user123',
  timestamp: Date.now()
}

const _testOutput: _MessageOutput = {
  content: 'hello',
  userId: 'user123',
  timestamp: Date.now()
}
